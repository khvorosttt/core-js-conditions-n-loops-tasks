...
function convertToRomanNumerals(num) {
  ...
  return convertTens(Math.floor(num / 10)) + convertUnits(num % 10);
}
...
function isContainNumber(num, digit) {
  let rest = num % 10;
  let tempNum = (num - rest) / 10;
  while (tempNum > 10) {
    if (rest === digit) {
      return true;
    }
    rest = tempNum % 10;
    tempNum = (tempNum - rest) / 10;
  }
  if (tempNum === digit) {
    return true;
  }
...
function getSpiralMatrix(size) {
  ...
  for (let k = 0; k < size / 2; k += 1) {
    ...
    for (let i = k + 1; i < size - k; i += 1) {
      matrix[i][size - 1 - k] = value;
      value += 1;
    }
    for (let i = size - 2 - k; i >= k; i -= 1) {
      matrix[size - 1 - k][i] = value;
      value += 1;
    }
    for (let i = size - 2 - k; i > k; i -= 1) {
      matrix[i][k] = value;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix[i].length; j += 1) {
      newMatrix[j][matrix.length - 1 - i] = initMatrix[i][j];
    }
  }
  return newMatrix;
}
...
function getNearestBigger(number) {
  ...
  while (Math.floor(tempNumber / 10) > 0) {
    numberArr[numberArr.length] = tempNumber % 10;
    tempNumber = Math.floor(tempNumber / 10);
  }
  numberArr[numberArr.length] = tempNumber;
  numberArr.reverse();
  ...
  let indexMinMax = index + 1;
  let minmax = numberArr[indexMinMax];
  for (let i = index + 1; i < numberArr.length; i += 1) {
    if (minmax > numberArr[i] && numberArr[i] > numberArr[index]) {
      minmax = numberArr[i];
      indexMinMax = i;
    }
  }
